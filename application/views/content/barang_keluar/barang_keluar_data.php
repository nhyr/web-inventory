                                                            ...
                                                            <td class="text-right">
                                                              <div class="btn-group" role="group" aria-label="Basic example">
                                                                <button type="button" 
                                                                  ...
                                                                  data-no_transfer_file="<?=$k['no_transfer_file']?>";
                                                                  data-l-no_transfer_file="<?=urlencode($k['no_transfer_file'])?>"
                                                                  data-tgl="<?=$tgl?>"
                                                                ...
                                                                <a  
                                                                  ...
                                                                  onclick = "window.open(`<?=base_url()?>barang_keluar/pdf_surat_jalan/<?=str_replace('/', '--',$k['no_transfer_file'])?>`, 'pdf_laporan_barang_stok', 'location=yes,height=700,width=1300,scrollbars=yes,status=yes'); "
                                                                >
                                                                  <i class="feather icon-file"></i>No Transfer File
                                                                </a>
                                                                <a  
                                                                  href="<?=base_url()?>barang_keluar/delete/<?=str_replace('/', '--',$k['no_transfer_file'])?>" 
        ...
        <div class="row">
          <div class="col-md-6">
            <div class="form-group">
              <label for="no_transfer_file">No Transfer File</label>
              <!-- <input type="text" class="form-control" id="no_transfer_file" name="no_transfer_file" placeholder="No Surat Jalan" maxlength="20" required> -->
              <input type="text" class="form-control" id="no_transfer_file" name="no_transfer_file" value="../KN/PBBF/VI/2022" maxlength="20" aria-describedby="validationServer03Feedback" required>
          ...
          <div class="col-md-6">
          <div class="form-group">
            <label for="id_user">Operator Penerima</label>
          ...
          <div class="row">
            <div class="col-6">
              <div class="form-group">
                <label for="no_batch_addr">No Batch & Nama Barang</label>
                <select class="form-control chosen-select" id="no_batch_add" name="no_batch_add" required>
                  ...
                  <?php 
                    foreach($bm as $s){
                  ?>
                  <option value="<?=$s['no_batch']?>,<?=$s['nama_barang']?>,<?=$s['stok']?>"><?=$s['no_batch']?> | <?=$s['nama_suplier']?> | <?=$s['stok']?><?=$s['satuan']?></option>
                  <?php
                    }
                  ?>
                </select>
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    $("#no_transfer_file").keyup(function(){
      var no_transfer_file =  $("#no_transfer_file").val();
      jQuery.ajax({
        url: "<?=base_url()?>barang_keluar/cek_transfer_file",
        dataType:'text',
        type: "post",
        data:{no_transfer_file:no_transfer_file},
        success:function(response){
          if (response =="true") {
            $("#no_transfer_file").addClass("is-invalid");
            $("#simpan").attr("disabled","disabled");
          }else{
            $("#no_transfer_file").removeClass("is-invalid");
      ...
      <div class="modal-body">
        <div class="row">
          <div class="col-md-6">
            <div class="form-group">
              <label for="no_transfer_file">No Transfer File</label>
              <input type="text" class="form-control" id="v_no_transfer_file" name="no_transfer_file" placeholder="No Surat Jalan" maxlength="20" readonly>
            </div>
          </div>
          <div class="col-md-6">
            <div class="form-group">
              <label for="v_tgl">Tanggal Keluar</label>
              <input type="text" class="form-control" id="v_tgl" name="tgl"  placeholder="Tanggal Keluar" readonly>
            </div>
          </div>
          <div class="col-md-6">
            <div class="form-group">
              <label for="id_user">Nama Operator</label>
              <input type="text" class="form-control" id="v_id_user" name="id_user"  placeholder="Nama Operator" readonly>
            </div>
          </div>
          <div class="col-md-6">
            <!-- <div class="form-group">
          ...
          <div class="col-md-6">
            <div class="form-group">
              <label for="no_po">No. PO</label>
              <input type="text" class="form-control" id="v_no_po" name="no_po"  placeholder="No PO" readonly>
            </div>
          </div>
          <div class="col-md-12">
            <div class="form-group">
              <label for="v_note">Keterangan</label>
              <textarea class="form-control" id="v_note" name="note" rows="3" readonly></textarea>
            </div>
          </div>
        </div>
        <div class="table-responsive">
...
$(document).ready(function() {
  ...
  var no_transfer_file = $(event.relatedTarget).data('no_transfer_file')
  var l_no_trasnfer_file = $(event.relatedTarget).data('l-no_transfer_file')
  ...
  $(this).find('#v_no_transfer_file').val(no_transfer_file)
  $(this).find('#v_tgl').val(tgl)
  // $(this).find('#v-exp').val(exp)
  $(this).find('#v_note').val(note)
  $(this).find('#v_id_user').val(id_user)
  $(this).find('#v_no_po').val(no_po)
    jQuery.ajax({
